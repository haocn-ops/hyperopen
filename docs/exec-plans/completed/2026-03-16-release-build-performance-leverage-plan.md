...
The dev/watch build still starts from the tracked shared `/hyperopen/resources/public/index.html`, which links `/css/main.css` directly and resolves the browser entry script from `/hyperopen/resources/public/js/manifest.json` before falling back to `/js/main.js`. The release artifact path now adds a generated `/hyperopen/out/release-public/index.html` that rewrites those references to parser-time fingerprinted CSS plus the hashed `main` script discovered from the release manifest. The browser build is configured in `/hyperopen/shadow-cljs.edn`, and the release/browser module outputs use hashed JS filenames through Shadow's manifest plus module-loader metadata.
