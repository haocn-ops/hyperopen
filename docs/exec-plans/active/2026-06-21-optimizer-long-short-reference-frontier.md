...
- [ ] User-side visual confirmation: open the long/short draft scenario in Spectate Mode (needs live Hyperliquid data), leave "Constrain Frontier" unchecked, and confirm the Target marker now sits on (not above) the curve. Then move this plan to `completed/`.
...
Remaining: the live, visual "Target sits on the frontier" confirmation in the user's spectate scenario (needs live Hyperliquid data) — tracked as the open Progress item.
