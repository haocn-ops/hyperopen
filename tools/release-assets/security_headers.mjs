...
export function buildContentSecurityPolicy({ imageSources = [], connectSources = [] } = {}) {
  const publicImageSources = [...new Set(imageSources)]
    .filter((source) => typeof source === "string" && /^https:\/\/[^\s]+$/i.test(source))
    .sort();
  const directives = [];
  ...
  appendDirective(directives, "img-src", [...DOCUMENT_IMG_SRC, ...publicImageSources]);
  appendDirective(directives, "connect-src", [
    ...DOCUMENT_CONNECT_SRC,
    ...normalizeHttpsOrigins(connectSources),
  ]);
...
export function expectedDocumentHeaders(options = {}) {
  return {
...
export function buildReleaseHeadersFile({
  ...
  const normalizedImmutableAssetPaths = [...new Set(immutableAssetPaths.map(normalizeImmutableAssetPath))]
    .sort();
  const documentHeaders = expectedDocumentHeaders({ imageSources, connectSources });
  const lines = [
