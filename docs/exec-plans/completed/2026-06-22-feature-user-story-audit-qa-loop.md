...
- Observation: `npm test` passes but still logs an existing `Aborted(OOM)` line inside `hyperopen.portfolio.optimizer.worker-test`.
  Evidence: The final `npm test` result was 4785 tests, 26470 assertions, 0 failures, 0 errors despite that log line.
...
The tracker milestone is complete: `/hyperopen/docs/qa/hyperopen-feature-user-story-tracker.xlsx` is the canonical feature status workbook. It contains 80 code-derived user-story rows across 17 feature areas, 24 documented defect/setup rows, a validation log, coverage hints, and source references. Every story row now has a direct baseline or retest status; the summary sheet reports zero baseline-not-run rows for every feature area.
