...
- [~] Phase 8 (in progress) — first near-duplicate pass (2026-06-10): 147 arbitrary utilities across 9 grey/teal families remapped onto tokens (text-secondary/text-muted/accent/accent-soft/accent-bright/sell-hi/bg/bg-deep/border; per-channel distance ≲16), baseline 1,644→1,489 literals / 119→110 files. Remaining intentionally unmapped: indicator style catalog (fixed multi-hue chart-line palette), pale teal-grey body text family (#dce9ee/#e6eff2/...; too far from tokens), modal border family. Staking/referrals/vaults deserve a design-eyeball under all three themes. Original scope: map the remaining long-tail color literals (near-duplicate grays/teals, `rgba(...)` arbitrary values, inline `:style` colors, modal/staking/referrals/vaults surfaces) onto the token vocabulary with design review via `npm run qa:design-ui`; ratchet baseline toward zero.
- [x] (2026-06-10) Phase 9 — live chart re-theme + token candles: candle-family series (candlestick/hollow/heikin-ashi/bar/columns) now read `--ho-chart-long/short` via `themed-series-color-options` (default values equal the old hardcoded #26a69a/#ef5350, pixel-identical), and each mounted chart attaches a `data-theme` MutationObserver (`runtime/observe-theme-changes!` → `ci/restyle-chart-theme!`) that re-applies layout/grid options plus main-series colors live. Volume bars keep per-bar colors until the next data sync. line/area/baseline/high-low keep their fixed palettes (tracked below).
- [~] Phase 10 (partial, 2026-06-10) — the document title brand segment is voiced (`app.document-title/title-for-state` reads `:brand/title`, so the live price-ticker tab title follows the theme). Favicon/loading-shell accents remain open.
