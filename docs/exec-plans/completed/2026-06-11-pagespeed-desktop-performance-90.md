...
Plan refresh note (2026-07-29): replaced the broad historical M0-M5 performance record with the user-approved Funding-only M2a execution contract. This avoids treating unapproved registry domains or PageSpeed follow-up as active work. A later refresh records the release-blocking responsive fix and the separately authorized GitHub/Cloudflare release evidence without broadening the implementation domain.
