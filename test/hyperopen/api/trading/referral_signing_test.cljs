(ns hyperopen.api.trading.referral-signing-test
...
(defn- assert-referral-action-omits-vault!
  [done {:keys [invoke! expected-action]}]
  (let [store (support/ready-agent-store 1700000040000)
        supplied-vault-address "0xABCDEF1234567890ABCDEF1234567890ABCDEF12"
    ...
    (set! signing/sign-l1-action-with-private-key!
          (api-stubs/signing-stub sign-calls))
    (try
      (-> (invoke! store
...
(deftest set-referrer-signs-master-scoped-action-without-vault-test
  (async done
    (assert-referral-action-omits-vault!
     done
     {:expected-action {:type "setReferrer"
                        :code "ABC123"}
      :invoke! (fn [store owner-address vault-address expires-after]
                 (trading/set-referrer! store
                                        owner-address
                                        "ABC123"
...
(deftest register-referrer-signs-master-scoped-action-without-vault-test
  (async done
    (assert-referral-action-omits-vault!
     done
     {:expected-action {:type "registerReferrer"
                        :code "MYCODE"}
      :invoke! (fn [store owner-address vault-address expires-after]
                 (trading/register-referrer! store
                                             owner-address
                                             "MYCODE"
                                             {:vault-address vault-address
                                              :expires-after expires-after
...
(deftest claim-rewards-signs-master-scoped-action-without-vault-test
  (async done
    (assert-referral-action-omits-vault!
     done
     {:expected-action {:type "claimRewards"}
      :invoke! (fn [store owner-address vault-address expires-after]
                 (trading/claim-rewards! store
                                         owner-address
                                         {:vault-address vault-address
                                          :expires-after expires-after
                                          :is-mainnet false
