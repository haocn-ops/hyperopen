...
- [x] (2026-07-16 23:05Z) Milestone 6 (user-requested during live testing): results analytics parity — GREEN. Engine: the risk-structure/contributions assembly was extracted from `equal_risk_payload` into the shared `application/engine/risk_analytics_payload.cljs`, and `inverse_volatility_payload` now emits top-level `:risk-contributions` (uniform 1/n summary targets, `:quality :diagnostic`), `:current-risk-contributions`, and `:risk-structure` beside the sizing section; `payload.cljs` widened the section merge. Gating audit: `equal-risk-result?` / `balance-model` / `verdict-body` now require objective-kind `:equal-risk` (nil-kind tolerated for persisted/fixture results), which closes every presence-inferred site at once (equal-risk confidence rails, impact strip, context card, KPI Risk-balance tile, verdict or-site); the trust rail's Negative-contributors swap and the Allocation table's P&L-correlation line stay presence-based by design. View: `risk_weighted_sizing_card` gained the sibling card's DOM-state radio tabs (SIZING FIDELITY default / CONTRIBUTIONS / DIVERSIFICATION / CORRELATION DRIVERS / RISK / RETURN) sharing the structure model's result-scoped radio naming; the CONTRIBUTIONS tab reuses the equal-risk card's lane primitives (made public) over a new target-free `contributions-model`, with no equal-target line/ticks, no Target/Deviation columns, no quality copy, and a diagnostic reading note; the DIVERSIFICATION / CORRELATION / RISK-RETURN tabs reuse the equal-risk panel bodies verbatim with minimal copy parameterization (risk-return `:objective-label`, diversification `:explainer`, `summary-tiles? false`, conditional breakdown target legend). The volatility-intuition rail already rendered for every solved result (no gate existed — verified and pinned by test). Suite: 5751 tests / 31798 assertions, 0 failures; `npm run gates` 34/34 PASS (6471 tests / 35184 assertions); tab `:has()` CSS verified against the live compiled stylesheet in the browser (default panel, per-radio switching, no cross-card leak in either direction).
...
- Decision: the sizing card reuses the risk-balance card's CSS grammar (shell, header, row grid, legend, reading note) but its body deliberately does NOT use `.optimizer-risk-balance-panel` — that class is display-gated by the sibling card's tab radios (`:has()` CSS) and the sizing card has no tabs. The "Why this target" context card stays rendered for this objective (engine facts incl. limits hit); only frontier machinery and equal-risk rails are gated off.
  Date/Author: 2026-07-16, Claude (GREEN implementer).
- Decision (Milestone 6): the shared analytics builder lives in a NEW `application/engine/risk_analytics_payload.cljs` rather than `inverse_volatility_payload` requiring `equal_risk_payload` directly.
  Rationale: the sections are objective-agnostic weights+covariance math with nothing Equal-Risk-specific; a neutral namespace keeps both payload namespaces well under their caps, names the ownership honestly, and leaves each objective's decoration (Equal Risk's convergence quality + solver metadata + warning codes, Risk-weighted sizing's `:quality :diagnostic` + silent-omit-on-error) in its own file. On alignment/degenerate inputs the inverse-vol payload OMITS the diagnostics without a warning — the sizing section is the objective's primary proof and never depends on them, whereas Equal Risk keeps its existing warning codes because the sections ARE its proof.
  Date/Author: 2026-07-16, Claude (Milestone 6 worker).
- Decision (Milestone 6): the equal-risk kind gates live in the view-model (`equal-risk-result?` requires kind `:equal-risk`, nil tolerated), not at each render site.
  Rationale: five surfaces inferred "equal risk" from `:risk-contributions` PRESENCE (confidence rail, quality rail, impact strip, context card, KPI tile via `balance-model`/`kpi` helpers, verdict or-site); one gate in the shared read model closes all of them and any future consumer. Nil-kind tolerance keeps persisted pre-kind payloads and hand-built view-model fixtures working — before this milestone only Equal Risk ever carried the section, so an unrecorded kind can only mean Equal Risk.
  Date/Author: 2026-07-16, Claude (Milestone 6 worker).
- Decision (Milestone 6): the sizing card's tabs reuse the SAME `data-risk-view` radio idiom, result-scoped radio names, and panel classes as the equal-risk card, with two new view values (`sizing`, `contributions`) and the card-scoped `.optimizer-risk-balance--sizing` modifier owning the default-panel rule.
  Rationale: the existing `:has()` CSS already keys panels on view VALUES, so reusing `breakdown`/`correlation`/`risk-return` verbatim gets those tabs for free (including the out-of-card label/for deep-link grammar); the modifier-scoped default rule makes the sizing view the no-radio-checked state without touching the sibling card's default-contribution rule, and the distinct `contributions` value keeps that rule inert on this card. Verified live in the browser against the compiled stylesheet.
  Date/Author: 2026-07-16, Claude (Milestone 6 worker).
- Decision (Milestone 6): the equal-risk impact strip ("What changes if you execute") is NOT adapted for :inverse-volatility — SKIPPED.
  Rationale: its lead chip (Risk imbalance, current → target RMS gap to the equal target) is balance-model-coupled and meaningless for an objective that does not target contributions; of the remaining two chips only Modeled volatility is unconditionally available (the 1-yr outcome chip is leverage-gated), and the strip's own honesty rule renders nil below two chips — so an inverse-vol variant would usually render nothing or a single-chip strip the strip itself forbids. The volatility story is already carried by the KPI strip's neutral current→target tile and the Diversification tab's decision summary.
  Date/Author: 2026-07-16, Claude (Milestone 6 worker).
...
Milestone 6 (2026-07-16 23:05Z). Files changed:
NEW `application/engine/risk_analytics_payload.cljs` (shared contribution/
structure builder), `application/engine/{equal_risk_payload,
inverse_volatility_payload,payload}.cljs`, `contracts/specs.cljs` (comments
only), `application/view_model/{equal_risk_results,
inverse_volatility_results}.cljs` (kind gates + shared `contribution-rows` +
`contributions-model`), `views/portfolio/optimize/{risk_weighted_sizing_card,
risk_contributions_card,risk_return_context,risk_diversification_summary,
risk_asset_breakdown_panel,risk_breakdown_panel,results_panel}.cljs`,
`src/styles/surfaces/optimizer/results.css` (sizing-card panel rules),
`dev/namespace_size_exceptions.edn` (specs.cljs 535, comment growth). Tests
extended (never weakened): `engine_inverse_volatility_test.cljs` (solved
payload carries :risk-contributions `:quality :diagnostic`,
:current-risk-contributions, :risk-structure; canonical spec still valid),
`results_panel_inverse_volatility_test.cljs` (fixture now carries the
diagnostic sections so every gating pin exercises the new payload shape; new
deftest pins all five tab labels, the contributions tab body, the absence of
target-tick/Target/Deviation data-roles, "Recommended (Risk-weighted
sizing)" over "Recommended (Equal Risk)", and the volatility-intuition rail).
`npm test` 5751/31798 0 failures; `npm run gates` 34/34 PASS (6471/35184);
lint:hiccup, lint:test, lint:namespace-sizes, test:styles (12/0),
lint:namespace-boundaries, lint:docs all pass. Browser evidence: the tab
`:has()` display gating was exercised against the live dev-server stylesheet
(computed-style probe: sizing panel default + first-tab highlight, per-radio
panel switching, equal-risk card default unaffected in both directions).
