...
(deftest persist-and-load-vault-index-cache-roundtrip-test
  (async done
    (browser-mocks/with-test-indexed-db
      (fn []
        (let [test-db-opts {:db-name "vault-index-cache-roundtrip-test"
                            :db-version 1
                            :store-names [indexed-db/vault-index-store]}]
          (with-redefs [platform/now-ms (fn []
            ...
            (-> (list-cache/persist-vault-index-cache-record!
                ...
                (.then (fn [persisted?]
                         (is (true? persisted?))
                         (-> (list-cache/load-vault-index-cache-record! test-db-opts)
                             (.then (fn [record]
                                      (is (= {:id "vault-index-cache"
                                              :version 1
                                              :saved-at-ms 1700000000000
                                              ...
                                              :rows [{:name "Alpha Vault"
                                                      ...
                                                      :snapshot-preview-by-key {:day {:series [1 2.5]
                                                                                     :last-value 2.5}}}]}
                                             record))
                                      (-> (list-cache/load-vault-index-cache-metadata! test-db-opts)
