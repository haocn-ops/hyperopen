...
- Decision: Balance floor on the fitted domain — once the book mixes signs, the smaller side gets at least 45% of the larger side's extent, so the zero origin lands ~30%+ across instead of hugging the plot edge when a hedge is tiny; purely one-sided books keep the full width for their bars.
  Rationale: user follow-up on the concentrated-current case — a −1% hedge left zero at 12.5% across and the chart read lopsided against the mock's balanced silhouette; a full symmetric domain would re-create the wasted-space complaint, 45% is the compromise that reads centered without halving bar resolution. Verified: concentrated scene zero 12.5% → 30.0%, designer-parity unchanged at 46.2% (its hedges already earn more than the floor).
  Date/Author: 2026-07-11 / centering follow-up.
