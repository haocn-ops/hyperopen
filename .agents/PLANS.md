...
Keep performance work evidence-driven. Do not justify an optimization because it "seems faster" or because a code path looks expensive. If the plan proposes performance-motivated work or a more complex algorithm or data structure, name the expected bottleneck, record the baseline measurement or profiling method that established it, state the workload assumptions that matter, and explain why a simpler change is insufficient.
...
* When work is justified by performance or throughput, record the baseline, the measurement method, the observed result after the change, and whether the extra complexity remained necessary.
