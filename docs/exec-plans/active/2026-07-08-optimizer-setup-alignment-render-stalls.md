...
- [ ] Owner confirmation on the real wallet/profile: goal-card switch now pays at most ONE covariance estimate (first switch only); confirm feel + web-vitals. If the single first-switch estimate (~1.2–1.9 s dev build, less in release) still reads slow, follow-ups are (a) prewarm the covariance memo off the critical path at the effect boundary after history settles, or (b) the owner's two-phase idea — paint the selection, then compute — which needs an async view-model seam.
