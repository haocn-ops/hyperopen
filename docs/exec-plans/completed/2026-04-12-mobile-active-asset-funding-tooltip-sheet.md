...
This document follows `/hyperopen/.agents/PLANS.md`, `/hyperopen/docs/PLANS.md`, and `/hyperopen/docs/WORK_TRACKING.md`. The live `bd` issue for this work is `hyperopen-yw1s` ("Patch mobile active-asset funding tooltip presentation"), and `bd` was the local tracker used at the time until this plan is moved out of `active`.
