...
- [ ] Follow-up — book-depth ratio bar under the order book (needs panel snapshot plumbing); slider threshold-crossing tick sounds; the prototype's purely-fake controls (YOLO toggles, weapons, DEGEN LVL, IQ chip, roulette, meme feed, coin modal, nav joke toasts) stay unimplemented by design in a real-money product.
