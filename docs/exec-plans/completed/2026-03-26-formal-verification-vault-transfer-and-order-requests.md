...
- [x] (2026-03-27 08:52 EDT) Completed the final mutation close-out. `/hyperopen/src/hyperopen/vaults/domain/transfer_policy.cljs` now has a fresh full `22/22` killed run. `/hyperopen/src/hyperopen/api/gateway/orders/commands.cljs` first reran full-module mutation and exposed one equivalent survivor in the unsupported-order default branch; the implementation was then simplified to remove that dead fallback, and the current changed-surface rerun now kills `23/23` selected mutants with `0` uncovered. `/hyperopen/src/hyperopen/domain/trading/market.cljs` now has a fresh canonicalization-range rerun at `13/13` killed on lines `155-202`. `/hyperopen/src/hyperopen/domain/trading/core.cljs` now has a fresh `scale-weights` rerun at `6/6` killed on lines `269-279`, plus a current scan of `92/92` covered. Acceptance is met and this plan is ready to move to `/hyperopen/docs/exec-plans/completed/`.
...
- Observation: the stubborn `scale-weights` mutation noise was real equivalence, and the cleanest fix was to move invalid-raw fail-closed handling to the collection level.
  Evidence: the intermediate rerun on `/hyperopen/src/hyperopen/domain/trading/core.cljs` surfaced the equivalent `L274 0 -> 1` mutant because all-invalid raw weights normalized back to the same uniform ladder. The implementation now computes `safe-weights` only when every raw weight is finite, the direct `js/NaN` skew seam test remains in `/hyperopen/test/hyperopen/state/trading/validation_and_scale_test.cljs`, the current scan reports `92/92` covered, and the narrowed rerun on lines `269-279` now kills `6/6`.
...
- Observation: mutation close-out depended on fresh coverage and was the last acceptance item to finish.
  Evidence: after rerunning `npm run coverage` from the final tree, the close-out completed with a fresh full `22/22` killed run on `/hyperopen/src/hyperopen/vaults/domain/transfer_policy.cljs`, a fresh changed-surface `23/23` killed rerun on `/hyperopen/src/hyperopen/api/gateway/orders/commands.cljs`, a fresh canonicalization-range `13/13` killed rerun on `/hyperopen/src/hyperopen/domain/trading/market.cljs`, and a fresh `scale-weights` `6/6` killed rerun on `/hyperopen/src/hyperopen/domain/trading/core.cljs`.
...
The final mutation slice did uncover two worthwhile cleanups before the work could close. In `/hyperopen/src/hyperopen/api/gateway/orders/commands.cljs`, the full-module rerun exposed an equivalent survivor in the unsupported-order wire-value fallback, so that dead permissive branch was removed rather than documented away. In `/hyperopen/src/hyperopen/domain/trading/core.cljs`, the `scale-weights` rerun exposed an equivalent per-leg invalid fallback, so the implementation now fails closed at the collection level and the direct seam test pins the `js/NaN` case explicitly. Those changes reduced conceptual noise without changing the intended money-moving behavior.

Overall complexity did go down. Hyperopen now trades a modest amount of optional proof-tooling complexity for a much clearer and reviewable definition of what vault and order actions are legal to emit, and the close-out mutation evidence is strong enough to support moving this plan to completed.
...
Plan update note (2026-03-27 08:52 EDT): Completed the mutation close-out across all four claimed kernels. The final evidence is a fresh full `22/22` vault run, a fresh `23/23` changed-surface commands rerun after removing an equivalent unsupported-order fallback, a fresh `13/13` market canonicalization rerun, and a fresh `6/6` `scale-weights` rerun after moving invalid-raw fallback handling to the collection level. Acceptance is met and the plan should move to completed.
