(ns hyperopen.views.portfolio.optimize.risk-return-context
  ...
  returns are informational and did not affect the weights. Rendered as the
  RISK / RETURN tab panel of the risk-contribution balance card, and
  deliberately NOT titled or drawn as a frontier: no curve was calculated and
  none is implied. Marker language matches the balance chart: green filled
  dot = recommended, gray outlined circle = current."
...
(defn- dot
  ...
  (let [left (str (x (:volatility point)) "%")
    ...
    (case (:kind point)
      :target
      [:div {:class ["optimizer-risk-balance-dot" "absolute" "-translate-x-1/2"
                     "translate-y-1/2"]
      ...
      :current
      [:div {:class ["optimizer-risk-balance-current" "absolute" "-translate-x-1/2"
                     "translate-y-1/2"]
