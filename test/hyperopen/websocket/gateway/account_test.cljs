(ns hyperopen.websocket.gateway.account-test
...
(deftest account-gateway-portfolio-fees-ledger-wrapper-coverage-test
  (let [called (atom [])
        post-info! (fn [& _] nil)]
    (with-redefs [account-endpoints/request-portfolio! (fn [& args]
                                                         (swap! called conj [:request-portfolio args])
                                                         {:ok :request-portfolio})
                  account-endpoints/request-user-fees! (fn [& args]
                                                         (swap! called conj [:request-fees args])
                                                         {:ok :request-fees})
                  account-endpoints/request-user-non-funding-ledger-updates! (fn [& args]
                                                                                (swap! called conj [:request-ledger args])
                                                                                {:ok :request-ledger})]
      (is (= {:ok :request-portfolio}
             (account-gateway/request-portfolio! {:post-info! post-info!}
                                                "0xabc"
                                                {:priority :high})))
      (is (= {:ok :request-fees}
             (account-gateway/request-user-fees! {:post-info! post-info!}
                                                "0xabc"
                                                {:priority :high})))
      (is (= {:ok :request-ledger}
             (account-gateway/request-user-non-funding-ledger-updates! {:post-info! post-info!}
                                                                        "0xabc"
                                                                        100
                                                                        200
                                                                        {:priority :high})))
      (is (some #(= :request-portfolio (first %)) @called))
      (is (some #(= :request-fees (first %)) @called))
      (is (some #(= :request-ledger (first %)) @called)))))
