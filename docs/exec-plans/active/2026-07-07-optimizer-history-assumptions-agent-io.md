...
- [x] (2026-07-07) Live-agent failure fix: a real external-LLM completion put instrument-ids in the `symbol` field (`{"symbol": "perp:AAVE"}`) and the import dropped every proxy → "3 invalid entries skipped". Resolution is now lenient (either field, either form, case-insensitive, namespaced-tail fallback — one `resolution-index` shared by targets and proxies); the instructions show both entry shapes; the zero-applied note breaks down WHY (incl. the unrecognized-proxy count it previously hid). Gates 34/34 (5,970 tests / 31,781 assertions); Playwright IO spec (now importing the malformed `{symbol: "perp:SOL"}` shape live) PASS ×2-each.
...
- Real agents do not respect field semantics even when the schema shows them: the first live completion wrote `{"symbol": "perp:AAVE"}` (the id in the symbol field) for every proxy. Import resolution must treat every reference value as id-or-symbol in any case; and a zero-applied note that says "invalid entries" without the dropped-proxy count reads as a mystery, not a diagnosis.
- No new backend endpoint needed for proxy day-counts: import resolves against the client's live catalog, and history-length validation happens post-import (prefetch → readiness → engine, surfaced as card warnings). A precomputed coverage list would only matter if a candidate menu returned to the file.
