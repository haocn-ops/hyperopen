...
- Default: device-scoped key storage (`localStorage`).
...
- Implemented in `/hyperopen/src/hyperopen/wallet/agent_session.cljs`, `/hyperopen/src/hyperopen/wallet/core.cljs`, `/hyperopen/src/hyperopen/core.cljs`, `/hyperopen/src/hyperopen/api/trading.cljs`, and `/hyperopen/src/hyperopen/utils/hl_signing.cljs`.
- Covered by `/hyperopen/test/hyperopen/wallet/agent_session_test.cljs`, `/hyperopen/test/hyperopen/wallet/core_test.cljs`, `/hyperopen/test/hyperopen/api/trading_test.cljs`, and Phase 1 flow tests in `/hyperopen/test/hyperopen/core_bootstrap_test.cljs`.
...
- Implemented in `/hyperopen/src/hyperopen/api/trading.cljs`, `/hyperopen/src/hyperopen/utils/hl_signing.cljs`, and `/hyperopen/src/hyperopen/core.cljs`.
- Added local agent-key L1 signing path and wired submit/cancel to agent-session signing with nonce cursor persistence and nonce retry behavior.
- Covered by `/hyperopen/test/hyperopen/api/trading_test.cljs`, `/hyperopen/test/hyperopen/utils/hl_signing_test.cljs`, and Phase 2 flow tests in `/hyperopen/test/hyperopen/core_bootstrap_test.cljs`.
...
- Implemented persistence mode preference load/save (`session` vs `local`) in `/hyperopen/src/hyperopen/wallet/agent_session.cljs`.
- Added wallet-menu persistence control and action/effect wiring in `/hyperopen/src/hyperopen/views/header_view.cljs` and `/hyperopen/src/hyperopen/core.cljs`.
- Added deterministic mode-switch hardening: switching persistence mode clears current agent session material and requires explicit re-enable with clear user messaging.
- Restored persisted mode during app init before wallet bootstrap in `/hyperopen/src/hyperopen/core.cljs`.
- Covered by new and expanded tests in `/hyperopen/test/hyperopen/wallet/agent_session_test.cljs`, `/hyperopen/test/hyperopen/core_bootstrap_test.cljs`, and `/hyperopen/test/hyperopen/views/header_view_test.cljs`.
...
- Added recovery-path and rollout-critical trading regressions in `/hyperopen/test/hyperopen/api/trading_test.cljs`, including:
  - cancel-path invalidation on missing API wallet errors,
  - explicit `Vault not registered` behavior coverage,
  - no silent session wipe for non-agent-invalidation responses.
- Added diagnostics redaction verification in `/hyperopen/test/hyperopen/websocket/diagnostics_sanitize_test.cljs`.
- Added QA execution matrix in `/hyperopen/docs/qa/agent-wallet-manual-matrix.md`.
- Added rollout/support runbook in `/hyperopen/docs/runbooks/agent-trading-rollout.md`.
- Added default-device persistence fallback coverage in `/hyperopen/test/hyperopen/views/header_view_test.cljs`.
...
- Mitigation: minimize key exposure, redact diagnostics/logging, support explicit `Session` mode for lower persistence risk.
