...
- [x] (2026-06-15) Reviewed re-ratchet 616,000 → 617,734 gz after post-M3 feature merges (spot trading + affordability/coherence fixes, optimizer draft→refine, spectate watchlist import/export, info-rate-limit diagnostics, USDH repay) grew release `main` past the M3 ceiling — the Cloudflare Pages deploy of `71a6d485` failed `lint:bundle-budget` at +1,671 gz (prod node22/jdk21 measured 617,671; local node24/jdk22 reproduction measured 617,734, committed as the higher ceiling so both toolchains pass). Confirmed honest growth, not a regression: no stray heavy production requires, and the M2b spec-strip still holds (`schema.contracts` absent from release main; only the dev-only `hyperopen.dev.contracts-preload`). Root cause of *silent* accumulation: the gate runs only in the release `build` (deploy), not PR CI (`tests.yml` runs test:ci/test:websocket only), so six merges crossed the ceiling unseen — wiring the budget gate into PR CI is the durable fix (see Decision Log).
- [ ] (Follow-up) Wire `lint:bundle-budget` (or an equivalent release-build size gate) into PR CI so main.js regrowth is caught at PR time, not at the Cloudflare deploy
...
- Decision (2026-06-15): re-ratchet the budget up to the measured current size rather than shrink main by ~1.7KB to fit under 616,000. Rationale: the +1,671 gz came entirely from already-merged, reviewed user-facing features (spot trading et al.), not a regression — the tool's own error message and `bundle-budget.json` policy sanction a reviewed re-ratchet for deliberate growth, and a deploy-unblocking byte-shaving exercise would be brittle and unrelated to the shipped work. Committed the higher local reproduction value (617,734) rather than the prod build's 617,671 so the ceiling covers both build toolchains; the ratchet stays tight (0 headroom) by design, deferring slack to the PR-CI gate follow-up.
