(ns hyperopen.views.trading-chart.utils.indicator-style-catalog
...
(def ^:private all-line-series-meta
...
(def ^:private all-histogram-series-meta
...
(defn- indicator-ids
  [definitions]
  (into #{} (map :id definitions)))

(def ^:private trend-indicator-ids
  (indicator-ids trend-catalog/trend-indicator-definitions))

(def ^:private structure-indicator-ids
  (indicator-ids structure-catalog/structure-indicator-definitions))

(def ^:private oscillator-indicator-ids
  (indicator-ids oscillators-catalog/oscillator-indicator-definitions))

(def ^:private volatility-indicator-ids
  (indicator-ids volatility-catalog/volatility-indicator-definitions))

(def ^:private flow-indicator-ids
  (indicator-ids flow-catalog/flow-indicator-definitions))

(def ^:private price-indicator-ids
  (indicator-ids price-catalog/price-indicator-definitions))

(defn- filter-series-meta
  [meta indicator-ids]
  (into {}
        (filter (fn [[[indicator-type _] _]]
                  (contains? indicator-ids indicator-type)))
        meta))

(defn- filter-marker-meta
  [meta indicator-ids]
  (into {}
        (filter (fn [[[indicator-type _] _]]
                  (contains? indicator-ids indicator-type)))
...
(def trend-line-series-meta
  (filter-series-meta all-line-series-meta trend-indicator-ids))

(def structure-line-series-meta
  (filter-series-meta all-line-series-meta structure-indicator-ids))

(def oscillator-line-series-meta
  (filter-series-meta all-line-series-meta oscillator-indicator-ids))

(def volatility-line-series-meta
  (filter-series-meta all-line-series-meta volatility-indicator-ids))

(def flow-line-series-meta
  (filter-series-meta all-line-series-meta flow-indicator-ids))

(def price-line-series-meta
  (filter-series-meta all-line-series-meta price-indicator-ids))

(def line-series-meta
  (merge trend-line-series-meta
         structure-line-series-meta
         oscillator-line-series-meta
         volatility-line-series-meta
         flow-line-series-meta
         price-line-series-meta))

(def trend-histogram-series-meta
  (filter-series-meta all-histogram-series-meta trend-indicator-ids))

(def structure-histogram-series-meta
  (filter-series-meta all-histogram-series-meta structure-indicator-ids))

(def oscillator-histogram-series-meta
  (filter-series-meta all-histogram-series-meta oscillator-indicator-ids))

(def volatility-histogram-series-meta
  (filter-series-meta all-histogram-series-meta volatility-indicator-ids))

(def flow-histogram-series-meta
  (filter-series-meta all-histogram-series-meta flow-indicator-ids))

(def price-histogram-series-meta
  (filter-series-meta all-histogram-series-meta price-indicator-ids))

(def histogram-series-meta
  (merge trend-histogram-series-meta
         structure-histogram-series-meta
         oscillator-histogram-series-meta
         volatility-histogram-series-meta
         flow-histogram-series-meta
         price-histogram-series-meta))

(def marker-meta
  (filter-marker-meta all-marker-meta structure-indicator-ids))
