...
- [x] (2026-03-09 18:31Z) Moved this plan to `/hyperopen/docs/exec-plans/deferred/` during ExecPlan lifecycle cleanup and recorded local `bd` issue `hyperopen-1ug` as historical scratch context.
...
This plan was never executed beyond authoring and discovery. It was moved out of `/hyperopen/docs/exec-plans/active/` during the 2026-03-09 lifecycle cleanup so that `active` only contains live execution work. If this parity work is resumed, use GitHub or a current active ExecPlan for durable tracking; local `bd` issue `hyperopen-1ug` is historical scratch context only.
