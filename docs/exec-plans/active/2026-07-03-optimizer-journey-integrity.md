...
- [x] (2026-07-03) Milestone 5 — implemented and validated (`npm run gates` 34/34, 5717 tests): the ACTIVE preset card discloses holdings-seeded constraints ("Constraints were seeded from your holdings — review them in Constraints below", warning tone, only when the universe source is :holdings AND the envelope matches no exposure preset), and the collapsed Constraints header reads "Custom · from holdings" instead of a bare "Custom". Scope note: the plan's suggestion that re-selecting the preset restores constraint defaults was wrong — `apply-portfolio-optimizer-setup-preset` sets objective + return model only, so the fix is pure disclosure; whether the manual holdings button should stop seeding constraints remains the open product question recorded in Plan of Work.
- [x] (2026-07-03) Browser QA: optimizer Playwright smoke 14/14 at `--workers=1` against the worktree static build. The smoke run surfaced a real over-reach in the first M1 cut (see Surprises) which was fixed before landing; one unrelated trade-regressions smoke failed in the serial batch and passes in isolation (the known cold-compile flake class).
- [ ] Maintainer review; then move this plan to `completed/` and file follow-up GitHub Issues for the deferred audit findings (vocabulary unification, entry-board rework, spectate toast aggregation, minor formatting).
...
- Observation (M1, caught by the optimizer Playwright smoke): "non-nil run scenario-id ⇒ saved scenario" is FALSE — unsaved drafts can carry synthetic ids (the smoke spec's retained draft uses `"draft-current"`), and `apply-worker-result` stamps `active-scenario :loaded-id` with the run's id on every success, so the discriminator must be "run id equals the PREVIOUSLY loaded scenario id" judged on the pre-message state. The first cut revealed `/portfolio/optimize/draft-current` and broke the retained-draft smoke; the corrected `scenario-bound?` predicate keeps synthetic-id unsaved runs on the `/draft` alias.
  Evidence: smoke failure `expected "/portfolio/optimize/draft" received "/portfolio/optimize/draft-current"`; new unit test `handle-worker-message-unsaved-synthetic-draft-id-still-reveals-draft-alias-test`; 14/14 smoke after the fix.
...
All five milestones implemented and validated on 2026-07-03 (`npm run gates` 34/34 — 5717 tests / 30781 assertions across the matrix; optimizer Playwright smoke 14/14 at `--workers=1`), landed as five commits on the worktree branch (M1 `3f87171f`, M2 `751e9fc0`, M3 `26a15f9a`, M4 `61d4a6a5`, M5 in the close-out commit). Pending maintainer review before this plan moves to `completed/`.

Against the purpose: (1) the save → edit → run sequence can no longer strand the user — every successful run reveals on a page that can render it, the draft alias is honestly idle instead of fake-loading, and Rerun is never an enabled no-op; (2) the commit point tells one story — sendable-legs dollar totals, per-row skip reasons with the interpolated band, a verdict count equal to the ledger's, and all-blocked plans no longer masquerade as "already at target"; (3) the seeding promise is either kept (identity-arrival re-attempt) or explicitly withdrawn ("No open positions to import"); (4) every trust badge is now an evaluation and warnings read as grouped plain language; (5) holdings-seeded constraints are disclosed on the glowing preset card.

Complexity: net near-neutral. Additive surface is small and view/view-model-side (one watcher mirroring an existing pattern, a `route-loading?` predicate, a grouped-warnings renderer, threshold/label helpers, one carried `:tolerance` field); no new actions/effects, no contract or persistence changes, two namespace-size budget adjustments. Two hypotheses from the audit were corrected by evidence during implementation (the persisted-empty-draft preseed poisoning, and preset-reapply restoring constraints) — both recorded in the Decision Log, which is exactly what the living-document sections are for.

Remaining (deferred by design): the six About Face leftovers plus this audit's deferred list (vocabulary unification, entry-board rework with timestamps, spectate toast aggregation on optimizer routes, signed-range formatting, frontier label collision, limits-hit chip affordance) → follow-up GitHub Issues; the single-workspace Phase-2 spike stays deferred.
