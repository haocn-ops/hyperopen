...
(defn copy-websocket-diagnostics!
  ...
  (let [state @store
        ...
        schedule-clear! (or schedule-copy-status-clear!
                            (fn [clear-fn delay-ms]
                              (js/setTimeout clear-fn delay-ms)))]
      ...
      (try
        (-> (.writeText clipboard* diagnostics-json)
            (.then (fn []
                     ...
                     (schedule-clear! #(set-copy-status! store nil) 1400)))
            (.catch (fn [err]
